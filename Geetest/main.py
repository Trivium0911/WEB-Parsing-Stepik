...
def main():
    ...
    with uc.Chrome(version_main=109, seleniumwire_options=proxy) as browser:
        ...
        if background:
            ...
            Если мы не используем простой слип, то скриншот может быть сделан на пару мгновений раньше необходимого, и дистанция не будет рассчитана.
        ...
        Используем встроенную функцию round() для округления числа, потому что нейросеть возвращает дробное число.
        '''
        ...
        # Следующие 4 строки выполняют операции над distance и наполняет список lst нобходимыми числами.
        num1 = distance // 100
        lst.extend(num1 * [100])
        num2 = distance % 100
        lst.append(num2)
